...
class SpectrumDecomposition:
    ...
    def transform(self, data: pd.DataFrame, freq=None):
        ...
        self.max_length = int(data.groupby("id").size().max())
        ...
        for id in data["id"].unique():
            # Fourier Transform
            sequence = data[data["id"] == id]
            sequence = sequence.sort_values("timeindex")
            ...
            decomposed_sequence["timeindex"] = sequence["timeindex"].values
            decomposed_sequence["id"] = id
